...
public class BadLinksChecker extends AFilesContentChecker {
    ...
    protected TheColumn getColumnToAddValueInto(TheReportModel theReportModel) {
        return theReportModel.findColumn(TheColumId.COL_REPO_SEC_BAD_LINKS_CHECKER);
    }

    @Override
    protected TheCellValue checkOneRepository(GitHubRepository repo, GitHubFacade gitHubFacade, Path repoDirectory) {
        Path filePath = Paths.get(repoDirectory.toString(), ".github", "workflows", "link-checker.yaml");
        String httpRef = repo.getHttpReferenceToFileInGitHub("/.github/workflows/link-checker.yaml");
